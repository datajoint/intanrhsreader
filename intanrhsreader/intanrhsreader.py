...
def read_header(fid):
    ...
    (header["desired_impedance_test_frequency"], header["actual_impedance_test_frequency"]) = struct.unpack(
        "<ff", fid.read(8)
    )
    ...
    (header["stim_step_size"], header["recovery_current_limit"], header["recovery_target_voltage"]) = struct.unpack(
        "fff", fid.read(12)
    )
    ...
    (header["dc_amplifier_data_saved"], header["eval_board_mode"]) = struct.unpack("<hh", fid.read(4))
                    ...
                    if signal_type == 0:
                        ...
                        if header["dc_amplifier_data_saved"]:
                            new_dc_channel = {
                                ...
                                "native_channel_name": "DC_" + new_channel["native_channel_name"],
                                "custom_channel_name": "DC_" + new_channel["custom_channel_name"],
                        ...
                        new_stim_channel = {
                            ...
                            "port_prefix": new_channel["port_prefix"],
                            "port_number": new_channel["port_number"],
                            "native_channel_name": "STIM_" + new_channel["native_channel_name"],
                            "custom_channel_name": "STIM_" + new_channel["custom_channel_name"],
                            "native_order": new_channel["native_order"],
                            "custom_order": new_channel["custom_order"],
                            "chip_channel": new_channel["chip_channel"],
                            "board_stream": new_channel["board_stream"],
...
def notch_filter(input, f_sample, f_notch, bandwidth):
    ...
    for i in range(2, L):
        output[i] = (
            a * b2 * input[i - 2] + a * b1 * input[i - 1] + a * b0 * input[i] - a2 * output[i - 2] - a1 * output[i - 1]
        ) / a0
...
def read_one_data_block(data, header, indices, fid):
    ...
    if header["num_amplifier_channels"] > 0:
        ...
        data["amplifier_data"][
            range(header["num_amplifier_channels"]), indices["amplifier"] : (indices["amplifier"] + 128)
    ...
    if header["num_board_adc_channels"] > 0:
        ...
        data["board_adc_data"][
            range(header["num_board_adc_channels"]), indices["board_adc"] : (indices["board_adc"] + 128)
    ...
    if header["num_board_dac_channels"] > 0:
        ...
        data["board_dac_data"][
            range(header["num_board_dac_channels"]), indices["board_dac"] : (indices["board_dac"] + 128)
        ] = tmp.reshape(header["num_board_dac_channels"], 128)

    if header["num_board_dig_in_channels"] > 0:
        data["board_dig_in_raw"][indices["board_dig_in"] : (indices["board_dig_in"] + 128)] = np.array(
            struct.unpack("<" + "H" * 128, fid.read(256))
        )

    if header["num_board_dig_out_channels"] > 0:
        data["board_dig_out_raw"][indices["board_dig_out"] : (indices["board_dig_out"] + 128)] = np.array(
            struct.unpack("<" + "H" * 128, fid.read(256))
        )
...
def load_file(filename, run_notch=True):
    ...
    if data_present:
        ...
        if header["dc_amplifier_data_saved"]:
            data["dc_amplifier_data"] = (
                np.zeros([header["num_amplifier_channels"], num_amplifier_samples], dtype=np.uint)
    ...
    if data_present:
        ...
        for i in range(header["num_board_dig_out_channels"]):
            data["board_dig_out_data"][i, :] = np.not_equal(
                np.bitwise_and(data["board_dig_out_raw"], (1 << header["board_dig_out_channels"][i]["native_order"])), 0
            )
